...
Phase 2: OAuth2 config with Authorization Code + PKCE and Client Credentials, JWT handling with JOSE, Refresh token in HttpOnly cookie with rotation and replay protection via Redis, Auth DTOs (records) and Mappers (MapStruct), AuthService/Controller, custom AuthException, integration tests (20 tests passing) with curl verification (5 scenarios tested successfully)
Phase 3: UserEntity with JSONB roles and @Version, AdminAuditLogEntity with JSONB old/new values, DTOs as records (UserCreateDTO, UserDTO, UserUpdateDTO, etc.), UserMapper with MapStruct, UserRepository and AdminAuditLogRepository, UserService with registration/password encoding/role assignment, AuditService with Spring event listeners, EmailService and VerificationTokenService for async email flows, DatabaseUserDetailsService for JPA-backed auth, AuthController extensions (register/forgot-password/reset-password), UserController and AdminUserController with RBAC via @PreAuthorize, AsyncConfig for non-blocking emails, StringSetConverter and JsonMapConverter for JSONB support, RateLimitService with Redis-based IP rate limiting (10/min login, 5/min register, 3/5min password reset), CSRF protection via SameSite=Strict cookies, integration tests (44 total tests passing)
Claude must not re-implement or alter these without instruction.

4.3 What Is IN PROGRESS

Nothing currently in progress. Ready to begin Phase 4.

4.4 What Is NEXT (Single Source of Truth)
Next Phase: Complete Phase 4 – Catalog Basics
Goals:

Implement ProductEntity with JSONB attributes, BigDecimal price, discount fields
Implement CategoryEntity with hierarchical support (parentId)
Create DTOs as records (ProductDTO, CategoryDTO) and MapStruct mappers
Implement ProductRepository with search capabilities
Add ProductService with @Cacheable Redis caching
Create ProductController for /api/v1/store/products endpoints
Set up CacheConfig for Redis
Integration tests with curl verification
...
8. Required Features Checklist (To Prevent Missing Vital Features)
   ...
   [x] JWT handling with Spring Security OAuth2 Jose, short-lived access (1 hour)
   [x] Refresh token in HttpOnly cookie, rotation with Redis invalidation
   [x] Auth DTOs as records (LoginRequest, TokenResponse, UserInfoResponse, etc.)
   [x] AuthMapper with MapStruct
   [x] AuthService: token issuance/validation, throw AuthException, load secrets from .env
   [x] AuthController: /oauth2/token, /api/v1/auth/login, /api/v1/auth/refresh, /api/v1/auth/logout, /api/v1/auth/me
   [x] JwtAuthenticationFilter for parsing and blacklist checking
   [x] Integration tests for flows (20 tests passing), curl verification
   [x] Vital: Replay protection via token family tracking, stateless access tokens, password grant rejected
   ...
   [x] DTOs as records: UserDTO, UserCreateDTO, UserUpdateDTO, RoleAssignDTO, ForgotPasswordRequest, ResetPasswordRequest, RegistrationResponse
   [x] UserMapper with MapStruct (ignore password on toDto)
   [x] Repositories: UserRepository (findByUsername/Email, searchUsers, findByRole), AdminAuditLogRepository
   ...
   [x] AuditService: logAction via Spring ApplicationEvent (AdminAuditEvent)
   [x] Controllers: AuthController extensions for register/forgot/reset/verify, UserController for profile, AdminUserController for admin operations
   [x] @PreAuthorize for RBAC (e.g., hasRole('ROLE_SUPER_ADMIN') for role assign, hasAnyRole for admin endpoints)
   [x] Audit logging: Use Spring events (AdminAuditEvent) to auto-log changes
   [x] Logout: Blacklist JWT in Redis, revoke refresh (existing from Phase 2)
   [x] Password recovery: Forgot/reset with Redis TTL UUID tokens via VerificationTokenService, async emails via EmailService
   [x] Email verification on register: Send link with token, validate via VerificationTokenService
   [x] Integration tests with H2 (PostgreSQL mode) and embedded Redis, 37 tests passing
   [x] DatabaseUserDetailsService for JPA-backed authentication
   [x] AsyncConfig for non-blocking email sending with @Async
   [x] StringSetConverter and JsonMapConverter for JSONB support without Hypersistence Utils (Hibernate 7 compatibility)
   [x] Vital: Rate limiting on auth endpoints via Redis-based RateLimitService (10 req/min login, 5 req/min register, 3 req/5min password reset)
   [x] CSRF protection via SameSite=Strict cookies, HttpOnly, Secure flags (stateless JWT doesn't require traditional CSRF tokens)
...
Update these workflows/diagrams as phases progress also when you are done with a task in a phase mark it as completed